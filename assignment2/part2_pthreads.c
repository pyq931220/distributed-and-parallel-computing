...
void *RedBlueComputation(void *threadarg){
    ...
    int n = my_data->n;
    int t = my_data->t;
    int subsize = my_data->subsize;
    int max_itrs = my_data->max_itrs;
    float percent = my_data->percent;
...
void sequential(int *seqboard, int n, int t, int k, int max_itrs){
...
int main(int argc, char *argv[]){
    ...
    for(i=0;i<NUM_THREADS;i++) {
        ...
        thread_data_array[i].subsize = subsize;
        thread_data_array[i].max_itrs = max_itrs;
        thread_data_array[i].percent = percent;
    ...
    sequential(seqboard, n, t, k, max_itrs);
    free(board);
    free(seqboard);
